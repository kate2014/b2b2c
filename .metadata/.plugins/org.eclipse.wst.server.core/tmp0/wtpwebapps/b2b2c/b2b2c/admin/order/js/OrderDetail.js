...
var OrderDetail = {
		...
		$("#cancel").bind("click",function() {
			...
			if(!disabled){
				...
				　　				 if(!disabled){
					...
					　　				self.cancelOrder(self.orderid,reason,savebtn);
				　　				 }
				　　			 }
				　　		}]
			　　		});
		...
		$("#ship").bind("click",function() {
				...
				　　				 		}
					...
					　			    if(!disabled){
									...
									if(confirm("确认发货？快递公司："+$("#logi").find("option:selected").text()+"  快递单号："+$("input[name='express']").val())){
	...
	initBtnStatus : function() {
		...
		// 订单状态为作废，则禁用所有钮
		if (this.orderStatus == OrderStatus.ORDER_CANCELLATION) {
	...
	complete : function(orderId) {
		var self = this;
		$.ajax({
			url : ctx+ '/order/complete.do?orderId=' + orderId,
			dataType : "json",
			success : function(responseText) {
				if (responseText.result == 1) {
					$.Loading.success(responseText.message);
	 ...
	 * 作废
	 */
	cancelOrder : function(orderId, canel_reason,savebtn) {
		var self = this;
		$.Loading.show("正在保存请稍候..");
		savebtn.linkbutton("disable");
		$.ajax({
