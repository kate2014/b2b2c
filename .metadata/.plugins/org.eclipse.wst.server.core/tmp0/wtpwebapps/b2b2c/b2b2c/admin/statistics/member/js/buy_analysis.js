...
function initBuyTimeDis(dateWhere) {
	...
	// ajax配置
	var options = {
		url : ctx + "/b2b2c/admin/memberStatistics/get-buy-time-dis.do" ,
...
function initBuyFre(dateWhere){
	...
	var store_id=$('#store option:selected') .val();//选中的值
	var options = {
		url : ctx + "/b2b2c/admin/memberStatistics/get-buy-fre.do",
		data : {'start_date' : dateWhere[0], 'end_date' : dateWhere[1],'store_id':store_id},
		type : "post",
		dataType:"json",
		success:function(data){
			...
			if (data.result == 1) {
				...
				var fre_data=data.data[0];
				var total_num=0;
				if(typeof(fre_data)!="undefined"){
					total_num=fre_data.total_num;
				}
				
				// 添加 超过频率的数据
				var lastRow = lastRow = {"total_num":total_num,"buy_num":buyFreNum + '+',"member_num":0};	// 最后一行数据
				for (var key in data.data) {
					var row = data.data[key];
...
function initOrderPrice(where,dateWhere){
	...
	// ajax配置
	var options = {
		url : ctx + "/b2b2c/admin/memberStatistics/get-order-price-dis.do?" + where,
		data : {'start_date' : dateWhere[0], 'end_date' : dateWhere[1],'store_id':store_id},
		type : "post",
		dataType:"json",
		success:function(data){
