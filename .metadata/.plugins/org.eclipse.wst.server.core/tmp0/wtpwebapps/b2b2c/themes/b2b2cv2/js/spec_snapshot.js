var Products=[];
var Spec={
	init:function(goodsid,snapshotid,product_id){
		var self = this;
		...
		this.loadProduct(goodsid,snapshotid,product_id);
	 
	},
	loadProduct:function(goodsid,snapshotid,product_id){
		var self= this;
		$.ajax({
			url:ctx+"/api/shop/goods/product-snapshot-list.do?goodsid="+goodsid+"&snapshotid="+snapshotid+"&product_id="+product_id,
			dataType:"json",
			...
			success:function(json){
				if(json.result==1){
					...
					$("#goodsform [name='action']").val("add-product");
			...
			if(!isLogin){
				...
				$.dialog({ title:'提示信息',content:html,lock:true,init:function(){
					...
					$(".button-wrapper .to-login-btn").click(function(){
						 location.href="store/login.html?forward=/b2b2c/goods-"+goodsid+".html";
					});

					//
					$(".button-wrapper .to-checkout-btn").click(function(){
						location.href="store/register.html?forward=/b2b2c/goods-"+goodsid+".html";
					});
...
var PriceRefresh={
	refresh:function(pro){
		if(pro.length==1){
			...
			$(".goodssn strong").html(pro[0].sn);     //更改商品货号
			$("#productid").val(pro[0].product_id);
		}
		else{
...
function canBuy(){
	$("input[name=action]").val("add-product");
	$("#buyNow").unbind('click');
