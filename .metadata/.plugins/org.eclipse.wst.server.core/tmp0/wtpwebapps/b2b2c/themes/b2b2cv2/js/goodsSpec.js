...
var SpecOper={
	...
	buttonBind:function(){
		...
		$("#specCloseBtn").click(function(){
			...
			if("0"!=goodsid){
				$.ajax({
					url:ctx+"/shop/admin/goods-spec/closeSpec.do?action=check-goods-in-order&ajax=yes&goodsid="+goodsid,
					dataType:"json",
	...
	createSpecHead:function(specnames){
		...
		thead.append("<th>货号<span class='red'>*</span></th><th>价格<span class='red'>*</span></th><th>重量<span class='red'>*</span></th><th>成本价</th>");
	...
	createSpecBody:function(specAr){
		...
		for(i in specAr){ 
			...
			for(j in childAr){ //这是一列
				...
				if(propid!="")propid+="_";
				propid+=spec.valueid;				
				tr.append($("<td>"+spec.value+"" +
						"<input name='specvalue_"+index+"' value='"+spec.value+"' type='hidden'>" +
						"</td>"));
			...
			if(sn==""){
				 td ='<td><input type="text" class="ipt form-control" maxlength="20"  onblur="sn_is_exist(this)" name="sns" value="'+nowsn+"-"+index+'" autocomplete="off" propid="'+propid+'" prop="sn"><span style="float:left;color:red;" class="err"></span>';
			}else{
				 td ='<td><input type="text" class="ipt form-control" maxlength="20"    onblur="sn_is_exist(this)" name="sns" value="'+sn+'" autocomplete="off" propid="'+propid+'" prop="sn"><span style="float:left;color:red;" class="err"></span>';
			}
			...
			if(price=="0"){ 
				...
				td+="<td>"+hidden+"<input class='ipt price form-control' maxlength='7'  propid='"+propid+"' prop='price' size='8'  value='"+nowPrice+"' type='text'  name='prices' />";
			}else{
				td+="<td>"+hidden+"<input class='ipt price form-control' maxlength='7' propid='"+propid+"' prop='price' size='8'  value='"+price+"' type='text'  name='prices' />";
			}
			if(weight=="0"){
				if(nowWeight==""){
					nowWeight="0";
				}
				td+='<td><input type="text" size="10" name="weights"  maxlength="7"  value="'+nowWeight+'" autocomplete="off" class="ipt form-control" propid="'+propid+'" prop="weight"></td>';
			}else{
				td+='<td><input type="text" size="10" name="weights" maxlength="7" value="'+weight+'" autocomplete="off" class="ipt form-control" propid="'+propid+'" prop="weight"></td>';
			}
			if(cost=="0"){
				if(nowCost==""){
					nowCost="0";
				}
				td+='<td><input type="text" size="8" name="costs"  maxlength="7"  value="'+nowCost+'" autocomplete="off"  class="ipt form-control" propid="'+propid+'" prop="cost"></td>';
			}else{
				td+='<td><input type="text" size="8" name="costs"  maxlength="7" value="'+cost+'" autocomplete="off"  class="ipt form-control" propid="'+propid+'" prop="cost"></td>';
			}
			tr.append($(td));
