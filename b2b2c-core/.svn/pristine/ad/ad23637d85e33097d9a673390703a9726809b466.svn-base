...
public abstract class StoreCartContainer {
	...
	//存放购物车已选商品列表的 key
	private static final String CART_SELECT_SESSION_KEY ="store_select_cart_list";
	...
	public static void cleanSession(){
		...
		ThreadContextHolder.getSession().removeAttribute(CART_SELECT_SESSION_KEY);
		ThreadContextHolder.getSession().removeAttribute(ADDRESS_SESSION_KEY);
	...
	public static List<Map> getStoreCartListFromSession(){
		return (List)ThreadContextHolder.getSession().getAttribute(CART_SESSION_KEY);
	}
	...
	public static void putStoreCartListToSession(List<Map> storeCartList){
		ThreadContextHolder.getSession().setAttribute(CART_SESSION_KEY, storeCartList);
	}
	 ...
	 * 如果没有进入过购物车列表会返回NULL
	 */
	public static List<Map> getSelectStoreCartListFromSession(){
		return (List)ThreadContextHolder.getSession().getAttribute(CART_SELECT_SESSION_KEY);
	}
	
	
	
	/**
	 * 向session中压入已选中购物车列表<br>
	 * 当进入到购物车列时需要调用此方法
	 * @param storeCartList 购物车列表
	 */
	public static void putSelectStoreCartListToSession(List<Map> storeCartList){
		ThreadContextHolder.getSession().setAttribute(CART_SELECT_SESSION_KEY, storeCartList);
	}
	...
	public static Map getStoreData(int storeid){
		...
		List<Map> storeCartList =  getSelectStoreCartListFromSession();
	...
	public static MemberAddress getUserSelectedAddress(){
		...
		return (MemberAddress)ThreadContextHolder.getSession().getAttribute(ADDRESS_SESSION_KEY);
	}
	...
	public static void putSelectedAddress(MemberAddress address){
		ThreadContextHolder.getSession().setAttribute(ADDRESS_SESSION_KEY, address);
