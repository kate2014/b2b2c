...
public class StoreGoodsTagManager implements IStoreGoodsTagManager {
	...
	public Page getGoodsList(Map map, int page, int pageSize) {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("select g.goods_id,g.name,r.tag_id,r.ordernum,g.price,g.buy_num,g.thumbnail,g.small,g.mktprice,g.big from es_tag_rel r LEFT JOIN es_goods g ON g.goods_id=r.rel_id where g.disabled=0 and g.market_enable=1");
		sql.append(" and r.tag_id=? ");
		sql.append(" order by r.ordernum desc");
