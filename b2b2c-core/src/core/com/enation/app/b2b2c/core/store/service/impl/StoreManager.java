...
public class StoreManager implements IStoreManager {
	...
	public Page store_list(Map other, Integer disabled, int pageNo, int pageSize) {
		...
		if (disabled.equals(-2)) {
			...
			sql.append("select (" + recently + ") recently , s.* from es_store s   where  disabled != " + disabled);
		} else {
			// 添加 查询到的列 recently 最近成交量
			sql.append("select (" + recently + ") recently ,s.* from es_store s   where  disabled = " + disabled);
		}
...
//		sql.append(" and s.store_id != 1");
	...
	@Override
	public List getStoreManagementGoodsType(Integer store_id) {
		if(store_id==null){
			return null;
		}
		String sql = "select goods_management_category from es_store where store_id=" + store_id;
