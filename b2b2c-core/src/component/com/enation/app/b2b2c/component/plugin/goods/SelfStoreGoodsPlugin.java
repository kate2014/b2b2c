...
import com.enation.app.shop.core.goods.plugin.IGoodsSearchFilter;
 ...
 */
@Component
public class SelfStoreGoodsPlugin extends AutoRegisterPlugin implements IGoodsBeforeAddEvent ,IGoodsBeforeEditEvent,IGoodsBackendSearchFilter,IGetGoodsAddHtmlEvent,IGetGoodsEditHtmlEvent{
...
//		if(adminUser!=null){
			...
			//如果平台与前台同时在同一浏览器上登陆，那么在商家中心修改商品时，也会得到后台管理员的信息，所以不可以用当前登陆的管理员信息做判断
			//change by DMRain 2016-2-23
			String goods_transfee_charge = request.getParameter("goods_transfee_charge");
			if (goods_transfee_charge != null) {
				goods.put("goods_transfee_charge",StringUtil.toInt(goods_transfee_charge, 0));
			}
